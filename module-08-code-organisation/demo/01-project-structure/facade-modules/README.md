...
`orders.js` needs the catalogue — but it doesn't `import` it. Instead, `run.js` creates the catalogue first and passes it into `createOrderService(catalogue)`. The order module only knows about the *interface* it receives, not where it came from.
